...
export default function YearListSidebar({ pageName, years }: { pageName: string; years: number[] }) {
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.side_content_area} ${isOpening ? styles.open : ''}`}>
        <div className={styles.close_icon_box}>
          <button onClick={() => setIsOpening(false)} className={styles.close_button}>
            <FontAwesomeIcon
              icon={faXmark}
              style={{
                display: 'block',
        ...
        <div className={styles.l_index}>
          <ul>
            <li>
              <Link href='#' className={styles.link_box} onClick={() => setIsOpening(false)}>
                {pageName}
              </Link>
            </li>
            {/* 数字を文字列へ変換しながら表示する */}
